...
source .env
forge build --zksync
echo "Compiling and deploying the  Token contract on ZKsync..."
ZKSYNC_TOKEN_ADDRESS=$(forge create src/MyToken.sol:MyToken --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account myDevKey --legacy --zksync | awk '/Deployed to:/ {print $3}')
...
ZKSYNC_POOL_ADDRESS=$(forge create src/MyTokenPool.sol:MyTokenPool --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account myDevKey --legacy --zksync --constructor-args ${ZKSYNC_TOKEN_ADDRESS} [] ${ZKSYNC_RNM_PROXY_ADDRESS} ${ZKSYNC_ROUTER} | awk '/Deployed to:/ {print $3}')
...
cast send ${ZKSYNC_TOKEN_ADDRESS} --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account myDevKey "grantMintAndBurnRole(address)" ${ZKSYNC_POOL_ADDRESS}
echo "Pool permissions set"

# Set the CCIP roles and permissions
echo "Setting the CCIP roles and permissions on ZKsync..."
cast send ${ZKSYNC_REGISTRY_MODULE_OWNER_CUSTOM} "registerAdminViaOwner(address)" ${ZKSYNC_TOKEN_ADDRESS} --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account myDevKey
cast send ${ZKSYNC_TOKEN_ADMIN_REGISTRY} "acceptAdminRole(address)" ${ZKSYNC_TOKEN_ADDRESS} --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account myDevKey
cast send ${ZKSYNC_TOKEN_ADMIN_REGISTRY} "setPool(address,address)" ${ZKSYNC_TOKEN_ADDRESS} ${ZKSYNC_POOL_ADDRESS} --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account myDevKey
echo "CCIP roles and permissions set"

# 2. On Sepolia!
foundryup
echo "Running the script to deploy the contracts on Sepolia..."
output=$(forge script ./script/Deployer.s.sol:TokenAndPoolDeployer --rpc-url ${SEPOLIA_RPC_URL} --account myDevKey --broadcast)
echo "Contracts deployed and permission set on Sepolia"

# Extract the addresses from the output
SEPOLIA_TOKEN_ADDRESS=$(echo "$output" | grep 'token: contract MyToken' | awk '{print $4}')
SEPOLIA_POOL_ADDRESS=$(echo "$output" | grep 'pool: contract MyTokenPool' | awk '{print $4}')
...
VAULT_ADDRESS=$(forge script ./script/Deployer.s.sol:VaultDeployer --rpc-url ${SEPOLIA_RPC_URL} --account myDevKey --broadcast --sig "run(address)" ${SEPOLIA_TOKEN_ADDRESS} | grep 'vault: contract Vault' | awk '{print $NF}')
...
forge script ./script/ConfigurePool.s.sol:ConfigurePoolScript --rpc-url ${SEPOLIA_RPC_URL} --account myDevKey --broadcast --sig "run(address,uint64,address,address,bool,uint128,uint128,bool,uint128,uint128)" ${SEPOLIA_POOL_ADDRESS} ${ZKSYNC_SEPOLIA_CHAIN_SELECTOR} ${ZKSYNC_POOL_ADDRESS} ${ZKSYNC_TOKEN_ADDRESS} false 0 0 false 0 0

# Deposit funds to the vault
echo "Depositing funds to the vault on Sepolia..."
cast send ${VAULT_ADDRESS} --value ${AMOUNT} --rpc-url ${SEPOLIA_RPC_URL} --account myDevKey "deposit()"

# Wait a beat for some interest to accrue

# Configure the pool on ZKsync
echo "Configuring the pool on ZKsync..."
#cast send ${ZKSYNC_POOL_ADDRESS}  --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account myDevKey "applyChainUpdates(uint64[],(uint64,bytes[],bytes,(bool,uint128,uint128),(bool,uint128,uint128))[])" "[${SEPOLIA_CHAIN_SELECTOR}]" "[(${SEPOLIA_CHAIN_SELECTOR},[$(cast abi-encode "f(address)" ${SEPOLIA_POOL_ADDRESS})],$(cast abi-encode "f(address)" ${SEPOLIA_TOKEN_ADDRESS}),(false,0,0),(false,0,0))]"
...
forge script ./script/BridgeToken.s.sol:BridgeTokensScript --rpc-url ${SEPOLIA_RPC_URL} --account myDevKey --broadcast --sig "run(address,uint64,address,uint256,address,address)" $(cast wallet address --account myDevKey) ${ZKSYNC_SEPOLIA_CHAIN_SELECTOR} ${SEPOLIA_TOKEN_ADDRESS} 9998 ${SEPOLIA_LINK_ADDRESS} ${SEPOLIA_ROUTER}
